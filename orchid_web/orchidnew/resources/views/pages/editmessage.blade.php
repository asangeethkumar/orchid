    ...
    <div class="row create_event">
        ...
        <form action="{{'/create-msg'}}" method="post">
            <div class="col-6 col-sm-6 col-md-offset-4 col-md-2 col-lg-offset-4 col-lg-2">
                <input type="submit" name="saveprofile" id="save_profile" class="saveprofile" value="Next">
            </div>
        </div>
    </div>
                            ...
                            <div class="recipent">
                                ...
                                <div id="main" class="row">
                                    <div class="col-8 col-sm-8 col-md-8 col-lg-8 datesel_val">
                                        @if($selectedcard['card_data']['recipient_email'] == "")
                ...
                <!--                            <input id="searchbtn" type="search" class="card_search" placeholder="Search name..." size="35"/>
                                        ...
                                        @else
                                            @if(($userrole == "SUPER ADMIN") || ($userrole == "ADMIN"))
                                                <input class="eventcreate" type="email" name="recipent_mail" size="35" placeholder="Recipent email" value="{{$selectedcard['card_data']['recipient_email'] }}"/>
                                            @elseif($userrole == "MEMBER")
                                                <input class="eventcreate" type="email" name="recipent_mail" size="35" placeholder="Recipent email" value="{{$selectedcard['card_data']['recipient_email'] }}" disabled="true"/>
                                            @else
                                                <input class="eventcreate" type="email" name="recipent_mail" size="35" placeholder="Recipent email" value="{{$selectedcard['card_data']['recipient_email'] }}" disabled="true"/>
                                            @endif
                                        @endif
                                    ...
                                    <div id="datesel" class="col-4 col-sm-4 col-md-4 col-lg-4 datesel_val">
                                        @if($selectedcard['card_data']['send_card_date'] == "")
                                            <input class="eventcreate" name="sendby" type="datetime" id="response_event" size="12" placeholder="Send by" autocomplete="off" required="true"/>
                                        @else
                                            @if(($userrole == "SUPER ADMIN") || ($userrole == "ADMIN"))
                                                <input class="eventcreate" name="sendby" type="datetime" id="response_event" size="12" placeholder="Send by" value="{{ Carbon\Carbon::parse($selectedcard['card_data']['send_card_date'])->format('d-M-Y') }}"/>
                                            @elseif($userrole == "MEMBER")
                                                <input class="eventcreate" name="sendby" type="datetime" id="response_event" size="12" placeholder="Send by" value="{{ Carbon\Carbon::parse($selectedcard['card_data']['send_card_date'])->format('d-M-Y') }}" disabled="true"/>
                                            @else
                                                <input class="eventcreate" name="sendby" type="datetime" id="response_event" size="12" placeholder="Send by" value="{{ Carbon\Carbon::parse($selectedcard['card_data']['send_card_date'])->format('d-M-Y') }}" disabled="true"/>
                                            @endif
                                        @endif
                                    ...
                                    <div id="main" class="row">
                                        <div class="col-9 col-sm-9 col-md-9 col-lg-9 write_msg">   
                                            ...
                                            @if(empty($mymsg))
                                                <textarea class="eventcreate" name="addmessage" cols="60" placeholder="Add message" required="true"></textarea>
                                            @else
                                                <textarea class="eventcreate" name="addmessage" cols="60" placeholder="Add message" ><?php echo $mymsg?> </textarea>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="col-3 col-sm-3 col-md-3 col-lg-3 card_msg">
                            <div class="row card_details">
                                @foreach($addevents['output'] as $event)
                                    ...
                                    <div class="col-6 col-sm-6 col-md-6 col-lg-6">
                                        @foreach($addevents['output'] as $creater)
                                            <h4 class="lett_bold">Created by</h4>
                                            @if(($creater['created_by_first_name'] == "''") || ($creater['created_by_first_name'] == null))
                                                <p>{{$creater['email']}}</p>
                                            @else
                                                <p>{{$creater['created_by_first_name']}}</p>
                                            @endif
                                        @endforeach
                                    </div>
                                @endforeach
